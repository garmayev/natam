...
class SpikController extends \yii\rest\Controller
{
	...
	{
		$token = \Yii::$app->request->cookies->getValue("token", $this->authorization());
		\Yii::$app->response->cookies->add(new Cookie(["name" => "token", "value" => $token]));
		if ( $token ) {
			return $token;
		} else {
			return null;
	...
	public function actionSubscribe()
	{
		...
		if (isset($units)) {
			...
			$subscribe = \Yii::$app->request->cookies->getValue("subscribe", $this->getSubscribtionId($ids, $token));
