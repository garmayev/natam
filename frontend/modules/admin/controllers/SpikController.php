...
class SpikController extends \yii\rest\Controller
{
	...
	private function authorization()
	{
		...
		if ($response["IsAuthenticated"] && $response["IsAuthorized"]) {
			...
			\Yii::$app->response->cookies->add(new Cookie(["name" => "token", "value" => $authToken, "expire" => $expire]));
			return $authToken;
		}
	}
	...
	public function actionLogout()
	{
		...
		\Yii::$app->response->cookies->remove("token", true);
		\Yii::$app->response->cookies->remove("subscribe", true);
	}
	...
	public function actionSubscribe()
	{
		...
		$ids = [];
		$subscribe = \Yii::$app->request->cookies->getValue("subscribe");
		if ( isset($subscribe) ) return $subscribe;
		$units = $this->units($token);
		if ( is_null($subscribe) && isset($units) ) {
			foreach ($units["Units"] as $unit) {
				$ids[] = $unit["UnitId"];
			}
			$subscribe = $this->getSubscribtionId($ids, $token);
			\Yii::$app->response->cookies->add(new Cookie(["name" => "subscribe", "value" => $subscribe]));
			return $subscribe;
		} else {
			return [];
		}
	...
	public function actionOnline()
	{
		...
		$onlineData = $this->getOnlineData($subscribe, $token);
		if (isset($onlineData["OnlineDataCollection"])) {
			$collection = $onlineData["OnlineDataCollection"];
