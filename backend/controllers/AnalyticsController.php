...
class AnalyticsController extends BaseController
{
    ...
    public function actionOrders($from_date = null, $to_date = null, $filter = null, $export = null)
    {
        ...
        if (is_null($from_date)) {
            $models->where(['>', 'created_at', strtotime("-1 month")]);
        } else {
            $models->andWhere(['>', 'created_at', Yii::$app->formatter->asTimestamp($from_date . " 23:59")]);
        }
        if (is_null($to_date)) {
            $models->andWhere(['<', 'created_at', time()]);
        } else {
    ...
    public function actionEmployee($from_date = null, $to_date = null, $filter = null, $export = null)
    {
        ...
        $orders = Order::find();
        if (is_null($from_date)) {
            $orders->where(['>', 'created_at', strtotime("-1 month")]);
        } else {
            $orders->andWhere(['>', 'created_at', Yii::$app->formatter->asTimestamp($from_date . " 23:59")]);
        }
        if (is_null($to_date)) {
            $orders->andWhere(['<', 'created_at', time()]);
        } else {
